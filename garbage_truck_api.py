...
"""
NewTaipeiTruckAPI - 支援以 lat/lng 做距離搜尋的新版
- 會先嘗試取得 data.ntpc.gov.tw 的 JSON（預設 URL）
- 以 item 中常見的經緯度欄位或 geometry 做距離計算
- 若無座標，fallback 為文字比對（區/路），再 fallback 為 synthetic
"""
        ...
        try:
            ...
            if "application/json" in ct or r.text.strip().startswith(("[","{")):
                ...
                try:
                    ...
                    logger.error("Unknown NTPC API response format")
                    return None
            ...
            try:
                ...
                logger.error("NTPC API request failed: %s status=%s preview=%s", e, status, preview)
            except Exception:
                logger.exception("NTPC API request failed")
            return None

    def get_schedules_by_location(self, lat: float, lng: float, radius_m: int = 2000, max_results: int = 8) -> List[Dict]:
        """
        以經緯度做篩選（主方法）。
        回傳每筆 dict 含 'car','location','time','city'，若 dataset 含經緯度會回傳距離排序結果。
        """
        # 1) 先抓 API 或快取
        ...
        if proximity_results:
            # sort by distance and return top max_results
        ...
        # 3) 若 dataset 無座標或半數無座標，fallback to text matching by parsed address
        # 作為 fallback，我們嘗試把附近的 district/road 用在文字比對 (這需要 caller 傳入 address)
