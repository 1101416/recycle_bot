...
class GarbageTruckAPI:
    def get_nearby_trucks(self, latitude: float, longitude: float, radius_km: float = 1.0) -> List[Dict]:
        ...
        nearby_trucks = []
        try:
            response = requests.get(GARBAGE_TRUCK_API_URL, timeout=15)
            response.raise_for_status() # 如果 API 狀態碼不是 200，會拋出錯誤
